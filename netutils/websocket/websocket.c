...
#include <netutils/mbed_sha1.h>
#include <netutils/mbed_base64.h>
#include <netutils/netlib.h>
#include <netutils/websocket.h>
#include <netutils/wslay.h>
 ...
 *
 * Description:
 *   1.execute wslay event recv and send.
 *   2.all valid data can read from callback.
 *
 * Input Parameters:
 *   server  - websocket_t server
 *
 * Returned Value:
 *   websocket_return_t.
 *
...
int websocket_handler(websocket_t *websocket)
{
	...
	while (websocket->state == WEBSOCKET_RUN_SERVER)
	{
		...
		if (r < 0) {
			...
			WEBSOCKET_ERR("[WS]:ERR:select function returned errno == %d\n", errno);
			return WEBSOCKET_SOCKET_ERROR;
		} else if (r == 0) {
			...
			if (FD_ISSET(fd, &read_fds)) {
				...
				if (r != WEBSOCKET_SUCCESS) {
					if(r!= WEBSOCKET_ERR_CALLBACK_FAILURE){
						WEBSOCKET_ERR("[WS] ERROR: fail to process recv event, result : %d\n", r);
					}
			...
			if (FD_ISSET(fd, &write_fds)) {
				...
				if (r != WEBSOCKET_SUCCESS) {
					if(r!= WEBSOCKET_ERR_CALLBACK_FAILURE){
						WEBSOCKET_ERR("[WS] ERROR: fail to process send event, result : %d\n", r);
					}
					websocket_update_state(websocket, WEBSOCKET_ERROR);
					return WEBSOCKET_CONNECT_ERROR;
				}
...
/****************************************************************************
 * Name: websocket_server_handshake
 *
 * Description:
 *   handshake with client steps.
 *   1.read all handshake file.
 *   2.authenticate it valid.
 *   3.give a valid response.
 *
 * Input Parameters:
 *   server  - websocket_t server
 *
...
int websocket_server_handshake(websocket_t *server)
{
	...
	if (header == NULL) {
		WEBSOCKET_ERR("[WS]:ERROR:fail to allocate memory for header\n");
		return WEBSOCKET_HANDSHAKE_ERROR;
	}
	...
	while (1) {
		...
		if (r < 0) {
			WEBSOCKET_ERR("[WS]:ERROR:fail to read socket errno = %d\n", errno);
			goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
		} else if (r == 0) {
			WEBSOCKET_ERR("[WS]:ERROR:HTTP Handshake: Got EOF\n");
			goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
		} else {
			header_length += r;
			if (header_length >= 4 && memcmp(header + header_length - 4, "\r\n\r\n", 4) == 0) {
				...
				WEBSOCKET_ERR("[WS]:ERROR:HTTP Handshake: Too large HTTP headers\n");
				goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
			}else{
				WEBSOCKET_ERR("[WS]:ERROR:HTTP Handshake: Reading HTTP headers\n");
			}
		}
	}

	if (strstr(header, "Upgrade: websocket") == NULL || strstr(header, "Connection: Upgrade") == NULL) {
		WEBSOCKET_ERR("[WS]:ERROR:HTTP handshake: Missing required header fields\n");
		goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
	}
	if ((keyhdstart = strstr(header, "Sec-WebSocket-Key: ")) == NULL) {
		WEBSOCKET_ERR("[WS]:ERROR:http_upgrade: missing required headers\n");
		goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
	}
	keyhdstart += 19;
	...
	keyhdend = strstr(keyhdstart, "\r\n");
	if (keyhdend == NULL) {
		WEBSOCKET_ERR("[WS]:ERROR:ttp_upgrade: missing required headers\n");
		goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
	}

	memset(client_key, 0, WEBSOCKET_CLIENT_KEY_LEN);
	if (keyhdend - keyhdstart > WEBSOCKET_CLIENT_KEY_LEN) {
		WEBSOCKET_ERR("[WS]:ERROR:error key length\n");
		goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
	}
	...
	while (header_sent < header_length) {
		...
		if (r < 0) {
			WEBSOCKET_ERR("[WS]:ERROR:fail to write socket errno = %d\n", errno);
			goto EXIT_WEBSOCKET_HANDSHAKE_ERROR;
		} else {
			header_sent += r;
		}
...
/****************************************************************************
 * Name: websocket_server_authenticate
 *
 * Description:
...
int websocket_server_authenticate(websocket_t *server)
{
	...
	if (websocket_server_handshake(server) != WEBSOCKET_SUCCESS) {
		WEBSOCKET_ERR("[WS]:ERROR:fail to handshake\n");
...
/****************************************************************************
 * Name: websocket_accept_handler
 *
 * Description:
 * 	 1. clean fd_set and set fd_set with listen_fd
 * 	 2. finds empty websocket server to deal with client
 * 	 3. copy current server data to emputy websocket server
 * 	 4. create a new pthread to deal with process.
 *
 * Input Parameters:
 *   init_server  - websocket server
 *
 * Returned Value:
 *   websocket_return_t.
 *
...
//		if (r == -1) {
//			if (errno == EINVAL) {
//				WEBSOCKET_ERR("[WS]:ERR:socket fd is not exist, init_server closing\n");
//				break;
//			}
//			if (errno == EINTR || errno == 0) {
//				continue;
//			}
//			WEBSOCKET_ERR("[WS]:ERR:init_server select function returned errno == %d\n", errno);
//			continue;
//		} else if (r == 0) {
//			for (i = 0; i < WEBSOCKET_MAX_CLIENT; i++) {
...
//					WEBSOCKET_ERR("[WS]:ERR:websocket server is inactive for %d msec, closing.\n", WEBSOCKET_SERVER_TIMEOUT);
//					r = WEBSOCKET_SUCCESS;
...
//				WEBSOCKET_ERR("[WS]:ERR:no more empty server table\n");
//				continue;
//			}
//
...
//			if (accept_fd < 0) {
//				WEBSOCKET_ERR("[WS]:ERR:Error in accept err == %d\n", errno);
//				r = WEBSOCKET_SOCKET_ERROR;
//				goto EXIT_ACCEPT;
//			}
//
//			if (websocket_config_socket(accept_fd) != WEBSOCKET_SUCCESS) {
//				WEBSOCKET_ERR("[WS]:ERR:fail to config socket\n");
//				r = WEBSOCKET_SOCKET_ERROR;
//				goto EXIT_ACCEPT;
//			}
//
...
//				WEBSOCKET_ERR("[WS]:ERR:fail to create thread, fd == %d\n", accept_fd);
//				r = WEBSOCKET_INIT_ERROR;
//				goto EXIT_ACCEPT;
//			}
//
//			if (pthread_setname_np(server_handler->thread_id, "websocket handler") != 0) {
//				WEBSOCKET_ERR("[WS]:ERR:fail to set thread name\n");
//				r = WEBSOCKET_INIT_ERROR;
//				goto EXIT_ACCEPT;
//			}
//
...
//				WEBSOCKET_ERR("[WS]:ERR:fail to detach thread\n");
//				r = WEBSOCKET_INIT_ERROR;
//				goto EXIT_ACCEPT;
//			}
//EXIT_ACCEPT:
 ...
 * 	 2. setting the socket
 * 	 3. binding with special ip address
 * 	 4. listenning.
 *
 * Input Parameters:
 *   listen_fd  - socket fd
...
int websocket_listen(int *listen_fd, int port)
{
	...
	*listen_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	if (*listen_fd == -1) {
		WEBSOCKET_ERR("[WS]:ERR:fail to open socket\n");
		return WEBSOCKET_SOCKET_ERROR;
	}

	if (setsockopt(*listen_fd, SOL_SOCKET, SO_REUSEADDR, &val, (socklen_t) sizeof(int)) == -1) {
		WEBSOCKET_ERR("[WS]:ERR:setsockopt fail\n");
	...
	if (bind(*listen_fd, (struct sockaddr *)&serveraddr, addrlen) == -1) {
		WEBSOCKET_ERR("[WS]:ERR:fail to bind socket\n");
		WEBSOCKET_CLOSE(*listen_fd);
		return WEBSOCKET_SOCKET_ERROR;
	}

	if (listen(*listen_fd, 16) == -1) {
		WEBSOCKET_ERR("[WS]:ERR:fail to listen socket\n");
		WEBSOCKET_CLOSE(*listen_fd);
		return WEBSOCKET_SOCKET_ERROR;
	}
...
//////////////////////////////////////////////////////////////////////////////
/****************************************************************************
 * Name: websocket_create_accept_key
 *
 * Description:
 *   handshake with client steps.
 *   1.read all handshake file.
...
int websocket_count_table(void)
{
	...
	for (i = 0; i < WEBSOCKET_MAX_CLIENT; i++) {
		if (ws_srv_table[i].state == WEBSOCKET_RUN_SERVER) {
			cnt++;
		}
	}
...
websocket_t * websocket_get_live_table(int *mask)
{
	int i;
	for (i = 0; i < WEBSOCKET_MAX_CLIENT; i++) {
		int bit = 1<<i;
...
websocket_t * websocket_get_server_table()
{
	return ws_srv_table;
}
...
websocket_t *websocket_find_table(void)
{
	...
	if (i >= WEBSOCKET_MAX_CLIENT) {
		WEBSOCKET_ERR("[WS]:ERR:too many websocket clients. limit : %d\n", WEBSOCKET_MAX_CLIENT);
		return NULL;
	}
...
{
	if (websocket == NULL) {
		WEBSOCKET_ERR("[WS]:ERR:NULL parameter\n");
		return WEBSOCKET_ALLOCATION_ERROR;
	}
	...
	if (state >= WEBSOCKET_STOP && state < WEBSOCKET_MAX_STATE) {
		...
		WEBSOCKET_ERR("[WS]:ERR:function returned for invalid parameter\n");
	}
 ...
 *
 * Description:
 *   alloc a websocket server and wait for client.
 * 	 1. allocating socket fd
 * 	 2. accepting client
 * 	 3. authenticating with client
 *
 * Input Parameters:
 *   init_server  - websocket_t server
 *
 * Returned Value:
 *   none.
 *
...
int websocket_accept_handler(websocket_t *init_server)
{
	...
	/* Reset the server handler state */
	for (i = 0; i < WEBSOCKET_MAX_CLIENT; i++) {
		ws_srv_table[i].state = WEBSOCKET_STOP;
	}

	/* Setup init server state */
	init_server->state = WEBSOCKET_RUN_SERVER;
	while (init_server->state != WEBSOCKET_STOP) {
		...
		if (r == -1) {
			if (errno == EINVAL) {
				WEBSOCKET_ERR("[WS]:ERROR:socket fd is not exist, init_server closing\n");
				break;
			}
			if (errno == EINTR || errno == 0) {
				continue;
			}
			WEBSOCKET_ERR("[WS]:ERROR:init_server select function returned errno == %d\n", errno);
			continue;
		} else if (r == 0) {
			for (i = 0; i < WEBSOCKET_MAX_CLIENT; i++) {
				if (ws_srv_table[i].state) {
			...
			if (i == WEBSOCKET_MAX_CLIENT) {
				timeout_cnt++;
				if (timeout_cnt * WEBSOCKET_ACCEPT_TIMEOUT >= WEBSOCKET_SERVER_TIMEOUT) {
					WEBSOCKET_ERR("[WS]:ERROR:websocket server is inactive for %d msec, closing.\n", WEBSOCKET_SERVER_TIMEOUT);
					r = WEBSOCKET_SUCCESS;
					goto EXIT_INIT_SERVER;
				}
			} else {
			...
			server_handler = websocket_find_table();
			if (server_handler == NULL) {
				WEBSOCKET_ERR("[WS]:ERROR:no free empty server,max client = 5\n");
				sleep(1);
				continue;
			}
			...
			accept_fd = accept(listen_fd, (struct sockaddr *)&clientaddr, &addrlen);
			if (accept_fd < 0) {
				WEBSOCKET_ERR("[WS]:ERROR:Error in accept err == %d\n", errno);
				r = WEBSOCKET_SOCKET_ERROR;
				continue;
			}
			/* Configure the socket */
			if (websocket_config_socket(accept_fd) != WEBSOCKET_SUCCESS) {
				WEBSOCKET_ERR("[WS]:ERROR:fail to config socket\n");
				r = WEBSOCKET_SOCKET_ERROR;
				goto EXIT_ACCEPT;
			}

			WEBSOCKET_DEBUG("[WS]:INFO:accept client, fd == %d\n", accept_fd);
			server_handler->fd = accept_fd;

			/* Authenticate client and shakehands */
			websocket_server_authenticate((pthread_addr_t) server_handler);
		}
	}
...
websocket_return_t websocket_client_open(websocket_t *client, char *host, char *port, char *path)
{
	...
	if (client == NULL || host == NULL || port == NULL || path == NULL) {
		WEBSOCKET_DEBUG("[WS]:INFO:NULL parameter\n");
		return WEBSOCKET_ALLOCATION_ERROR;
	}
	...
	if (websocket_client_handshake(client, host, port, path) != WEBSOCKET_SUCCESS) {
		WEBSOCKET_DEBUG("[WS]:INFO:fail to http handshake\n");
		r = WEBSOCKET_HANDSHAKE_ERROR;
		goto EXIT_CLIENT_OPEN;
	}
	...
	if (socket_data == NULL) {
		WEBSOCKET_DEBUG("[WS]:INFO:fail to allocate memory\n");
		r = WEBSOCKET_ALLOCATION_ERROR;
		goto EXIT_CLIENT_OPEN;
	}
	...
	if (wslay_event_context_client_init(&client->ctx, client->cb, socket_data) != WEBSOCKET_SUCCESS) {
		WEBSOCKET_DEBUG("[WS]:INFO:fail to init websocket client context\n");
		WEBSOCKET_FREE(socket_data);
		r = WEBSOCKET_INIT_ERROR;
		goto EXIT_CLIENT_OPEN;
	}

	WEBSOCKET_DEBUG("[WS]:INFO:start websocket client handling thread\n");

	if (pthread_attr_init(&client->thread_attr) != 0) {
		WEBSOCKET_DEBUG("[WS]:INFO:fail to init pthread attribute\n");
		r = WEBSOCKET_ALLOCATION_ERROR;
		goto EXIT_CLIENT_OPEN;
	}

	if (pthread_attr_setstacksize(&client->thread_attr, WEBSOCKET_STACKSIZE) != 0) {
		WEBSOCKET_DEBUG("[WS]:INFO:fail to set stack size\n");
		r = WEBSOCKET_ALLOCATION_ERROR;
		goto EXIT_CLIENT_OPEN;
	}
	ws_sparam.sched_priority = WEBSOCKET_PRI;
 ...
 ****************************************************************************/
/****************************************************************************
 * Name: websocket_server_init
 *
 * Description:
 *   1.initialize wslay event context server.
 *   2.start websocket server handling loop
 *
 * Input Parameters:
 *   server  - websocket_t server
 *
...
websocket_return_t websocket_server_init(websocket_t *server)
{
	...
	if (server == NULL) {
		WEBSOCKET_ERR("[WS]:ERROR:null websocket object\n");
		return WEBSOCKET_ALLOCATION_ERROR;
	}

	socket_data = calloc(1, sizeof(struct websocket_info_t));
	if (socket_data == NULL) {
		WEBSOCKET_ERR("[WS]:ERROR:fail to allocate memory\n");
		r = WEBSOCKET_ALLOCATION_ERROR;
	...
	if (wslay_event_context_server_init(&(server->ctx), server->cb, socket_data) != WEBSOCKET_SUCCESS) {
		WEBSOCKET_ERR("[WS]:ERROR:fail to initiate websocket server\n");
		r = WEBSOCKET_INIT_ERROR;
		goto EXIT_SERVER_INIT;
	}
	...
	WEBSOCKET_DEBUG("[WS]:INFO:start websocket server handling loop\n");
...
websocket_return_t websocket_server_open(websocket_t *init_server)
{
	...
	//is null object?
	if (init_server == NULL) {
		WEBSOCKET_ERR("[WS]:ERR:NULL init_server object.\n");
		return WEBSOCKET_ALLOCATION_ERROR;
	}

	//TODO : all TLS initializing code will be moved from example to here

	port = init_server->tls_enabled ? 443 : 8080;
	...
	WEBSOCKET_DEBUG("[WS]:INFO:websocket listen on fd == %d, port == %d\n", init_server->fd, port);
